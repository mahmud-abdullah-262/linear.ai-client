...
import { CriticalPulseDot } from '@/components/shared/MotionWrapper';
...
export function MetricCard({ label, value, sub, icon, accent = 'text-cyan-400' }: MetricCardProps) {
  return (
    <motion.div
      ...
      className="bg-[#1E293B] border border-slate-700/60 rounded-2xl p-5 flex flex-col gap-3 min-h-[120px] hover:border-cyan-500/30 transition-colors duration-200 cursor-default"
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs font-semibold ${color}`}>
      {priority === 'Critical' ? (
        ...
        <CriticalPulseDot className={`w-1.5 h-1.5 rounded-full ${dot} shrink-0`} />
      ) : (
...
export function TaskRow({ task, showAssignee = true }: { task: Task; showAssignee?: boolean }) {
  return (
    <motion.div
      ...
      className="flex items-start gap-3 py-3 border-b border-slate-800/70 last:border-0 px-2 -mx-2 rounded-lg transition-colors duration-150"
    >
...
export function SectionCard({
  ...
  return (
    <motion.div
      ...
      className="bg-[#1E293B] border border-slate-700/60 rounded-2xl p-5 flex flex-col gap-4"
    >
